...
$script:moduleName = 'NewPolicy'

Describe "NewPolicy Tests" {

    BeforeAll {
        $resourceModulePath = Join-Path -Path (Split-Path $PSScriptRoot -Parent) -ChildPath "Modules\NewPolicy\DscResources\NewPolicy\NewPolicy.psm1"
        Import-Module -Name $resourceModulePath -Force
    }

    InModuleScope 'NewPolicy' {

        Context 'module manifest' {

            It 'Has a PowerShell module manifest that meets functional requirements' {
                Test-ModuleManifest -Path .\Modules\NewPolicy\NewPolicy.psd1 | Should Not BeNullOrEmpty
                $? | Should Be $true
            }
        }

        Context "NewPolicy\Get-Function1" {
        ...
        Context "NewPolicy\Set-TargetResource" {
        ...
        Context 'when the system is in the desired state' {
            ...
            Context "NewPolicy\Get-TargetResource" {
            ...
            Context "NewPolicy\Test-TargetResource" {
